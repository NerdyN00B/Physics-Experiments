...
class EN_Measure_In_Time(QMainWindow):
    ...
    def __init__(self):
        ...
        self.chunkSize=50
        self.curveCH1 = self.plotItem0.plot(pen='r', name = "Channel 0")
        self.curveCH2 = self.plotItem0.plot(pen='b', name = "Channel 1")
    ...
    def initTask(self):
        try:
            if not self.isMeasuring:
                ...
                self.task.ai_channels.add_ai_voltage_chan(f"{selected_device}/ai1")
    ...
    def updateGraph(self):
        ...
        if len(self.xdata) == len(self.dataCH1) and len(self.xdata) == len(self.dataCH2):
            self.curveCH1.setData(self.xdata[-viewSampleSize:],self.dataCH1[-viewSampleSize:])
            self.curveCH2.setData(self.xdata[-viewSampleSize:],self.dataCH2[-viewSampleSize:])
    ...
    def clearData(self):
        self.xdata, self.dataCH1, self.dataCH2 = [], [], []
        self.updateGraph()
    ...
    def updateData(self,task_handle, every_n_samples_event_type, number_of_samples, callback_data):
        ...
        newdataCH1 = newdata[0]
        newdataCH2 = newdata[1]
        self.reads += 1
        self.dataCH1.extend(newdataCH1)
        self.dataCH2.extend(newdataCH2)
        ...
        self.xdata.extend(timedata.tolist())
    ...
    def saveData(self):
        ...
        dataArray = np.array([self.xdata,self.dataCH1, self.dataCH2]).transpose()
        ...
        try:
            np.savetxt(dest,dataArray,"%.{}f".format(self.decimals),delimiter = ",",header = "axis 0 is time, axis 1 is Voltage on channel 0, axis 2 voltage on channel 1")
            savemsg = QMessageBox()
